...
final class PakArchiveCoreTests: XCTestCase {
    ...
    func testFormatInspectorIdentifiesAlternateQuakeCPrograms() {
        var data = Data()
        for value in [6, 5927, 60, 20, 0, 4, 0, 2, 0, 3, 60, 32, 0, 8, 5] {
            appendInt32(value, to: &data)
        }

        let client = Dictionary(uniqueKeysWithValues: PakFormatInspector.details(
            fileName: "csprogs.dat",
            data: data,
            fileSize: data.count
        ).map { ($0.label, $0.value) })
        XCTAssertEqual(client["Format"], "Compiled QuakeC program")
        XCTAssertTrue(client["Purpose"]?.contains("Client-side QuakeC") == true)

        let menu = Dictionary(uniqueKeysWithValues: PakFormatInspector.details(
            fileName: "menu.dat",
            data: data,
            fileSize: data.count
        ).map { ($0.label, $0.value) })
        XCTAssertEqual(menu["Functions"], "3")
        XCTAssertTrue(menu["Purpose"]?.contains("menu program") == true)
    }

    func testFormatInspectorUnderstandsQSSMListsAndBackups() {
        let packageList = Data("// Generated by PAK Loading Menu\nad_v1_80p1.pk3\npatch.pak\n".utf8)
        let list = Dictionary(uniqueKeysWithValues: PakFormatInspector.details(
            fileName: "pak.lst",
            data: packageList,
            fileSize: packageList.count
        ).map { ($0.label, $0.value) })
        XCTAssertEqual(list["Format"], "Package load-order list")
        XCTAssertEqual(list["Lines"], "3")
        XCTAssertTrue(list["Purpose"]?.contains("package load order") == true)
        XCTAssertEqual(
        ...
        for name in namedBackups {
            let details = Dictionary(uniqueKeysWithValues: PakFormatInspector.details(
                fileName: name,
                data: Data("{}\n".utf8),
                fileSize: 3
            ).map { ($0.label, $0.value) })
            XCTAssertNotNil(details["Purpose"], name)
        }

        let config = Dictionary(uniqueKeysWithValues: PakFormatInspector.details(
            fileName: "config-07-26-2026.cfg",
            data: Data("bind w +forward\n".utf8),
            fileSize: 16
        ).map { ($0.label, $0.value) })
        XCTAssertTrue(config["Purpose"]?.contains("dated backup") == true)

        let ipLog = Dictionary(uniqueKeysWithValues: PakFormatInspector.details(
            fileName: "iplog.dat",
            data: Data(repeating: 0, count: 60),
            fileSize: 60
        ).map { ($0.label, $0.value) })
        XCTAssertEqual(ipLog["Format"], "ProQuake IP log")
        XCTAssertEqual(ipLog["Entries"], "3")
        XCTAssertTrue(ipLog["Purpose"]?.contains("IP-prefix") == true)
        XCTAssertEqual(
            PakFormatInspector.summary(
                fileName: "iplog.dat",
                data: Data(repeating: 0, count: 60),
